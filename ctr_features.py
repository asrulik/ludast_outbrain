...
document_on_advertiser_ctr = train.groupby(['advertiser_id','document_id','ads_on_doc']).clicked.agg({'clicked' : 'sum', 'total' : 'count'}).reset_index()

#normalized CTR of campaign coupled with displays' document
document_on_campaign_ctr = train.groupby(['campaign_id','document_id','ads_on_doc']).clicked.agg({'clicked' : 'sum', 'total' : 'count'}).reset_index()


#create all of the final tables using ctr function
ad_ctr = ctr(ad_ctr, ['ad_id'], 'ad')

ad_ctr.to_csv(path_b + 'ad_ctr.csv', index = False)

ad_document_ctr = ctr(ad_document_ctr, ['ad_document_id'], 'ad_doc')

ad_document_ctr.to_csv(path_b + 'ad_document_ctr.csv', index = False)

document_on_ad_ctr = ctr(document_on_ad_ctr, ['document_id', 'ad_id'], 'docXad')

document_on_ad_ctr.to_csv(path_b + 'document_on_ad_ctr.csv', index=False)

document_on_ad_document_ctr = ctr(document_on_ad_document_ctr, ['document_id', 'ad_document_id'], 'docXad_doc')

document_on_ad_document_ctr.to_csv(path_b + 'document_on_ad_document_ctr.csv', index=False)

advertiser_ctr = ctr(advertiser_ctr, ['advertiser_id'], 'adv')

advertiser_ctr.to_csv(path_b + 'advertiser_ctr.csv', index = False)

campaign_ctr = ctr(campaign_ctr, ['campaign_id'], 'camp')

campaign_ctr.to_csv(path_b + 'campaign_ctr.csv', index = False)

document_on_advertiser_ctr = ctr(document_on_advertiser_ctr, ['document_id', 'advertiser_id'], 'docXadv')

document_on_advertiser_ctr.to_csv(path_b + 'document_on_advertiser_ctr.csv', index = False)
